...
[![Current](https://img.shields.io/badge/Current-2.6.10-blue.svg)](ROADMAP.md)
[![Next](https://img.shields.io/badge/Next-2.7.0-purple.svg)](ROADMAP.md)
...
- **Redis or Valkey backend** - OS-aware selection with existing-Redis preservation
- **Provider controls** - package-tier availability, reseller quotas, exposure modes (report-only by default)
- **Security & licensing** - abuse detection, AUTH brute-force protection, signed database-backed licensing with WHMCS automation
...
| Item | Status |
|---|---|
| Current release | `v2.6.10` |
| Release line | `v2.6.x - Security, Abuse Prevention and Licensing Automation` |
| Next milestone | `v2.7.x - Observability and Debugging` |

Recent updates:

- `v2.6.10` - WHM config changes keep the per-account config owned by the account user, so memory and other changes apply reliably
- `v2.6.8 / v2.6.9` - RDB snapshots off by default under load, multi-source server IP detection, dashboard metrics resilience, diagnostics export
- `v2.6.0-2.6.7` - Security & abuse prevention (report-only), AUTH protection, signed + database-backed licensing with WHMCS automation, ~93× faster WHM dashboard, branding presets
- `v2.5.0` - Provider enforcement & monetization, reseller quotas, guided Redis ↔ Valkey migration
- `v2.4.x` - Redis and Valkey compatibility foundation
